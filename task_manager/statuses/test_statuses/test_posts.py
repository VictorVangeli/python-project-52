...
class TestCreateStatus(StatusTestCase):
    def test_create_valid_status(self) -> None:
        """
        Test that a new status is successfully created via the create view
            with valid data.
        Verifies redirection, status count increment, and correctness of the
            created status.
        """
    ...
    def test_create_fields_missing(self) -> None:
        ...
        Verifies that form errors are returned and no new status is added to
            the database.
        """
    ...
    def test_create_status_exists(self) -> None:
        """
        Test that status creation fails when a status with the same name
            already exists.
        Verifies that a validation error is shown and no duplicate status is
            created.
        """
    ...
    def test_create_long_field(self) -> None:
        """
        Test that status creation fails when the 'name' field exceeds the
            maximum allowed length.
        Verifies that a validation error is returned and no new status is
            created.
        """
        status_data = self.test_status["create"]["valid"].copy()
...
class TestUpdateStatus(StatusTestCase):
    def test_update_status(self) -> None:
        """
        Test that an existing status can be successfully updated with valid
            data.
...
class TestDeleteStatus(StatusTestCase):
    def test_delete_status(self) -> None:
        ...
        Verifies redirection and confirms the status is removed from the
            database.
        """
        response = self.client.post(
    ...
    def test_delete_status_not_logged_in(self) -> None:
        ...
        Test that an unauthenticated user is redirected to the login page
            when attempting to delete a status.
