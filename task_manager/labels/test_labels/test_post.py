...
class TestCreateLabel(LabelTestCase):
    def test_create_valid_label(self) -> None:
        ...
        response = self.client.post(
            reverse_lazy("label_create"), data=label_data
        )
        ...
        self.assertEqual(Label.objects.last().name, label_data["name"])
    ...
    def test_create_label_exists(self) -> None:
        """
        Test that label creation fails when a label with the same name already
            exists.
        Verifies that a validation error is shown and no duplicate label is
            created.
        """
        ...
        self.assertIn("name", errors)
        self.assertEqual(
            [_("Label with this Name already exists.")], errors["name"]
        )
...
class TestDeleteLabel(LabelTestCase):
    def test_delete_label(self) -> None:
        ...
        Verifies redirection and confirms the label is removed from the
            database.
        """
        response = self.client.post(
